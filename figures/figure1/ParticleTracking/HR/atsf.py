...
dirread_U = '/projects/0/acc/pop/parcels/tx0.1/p21a.EO38Ma.tx0.1.%s_control/tavg/'%(config)
...
def set_the_fieldset(ufiles, bfile):
    ...
    fieldset.U.lon[2000:][fieldset.U.lon[2000:]<0] += 360
    fieldset.V.lon[2000:][fieldset.V.lon[2000:]<0] += 360
    fieldset.W.lon[2000:][fieldset.W.lon[2000:]<0] += 360
    fieldset.T.lon[2000:][fieldset.T.lon[2000:]<0] += 360
    fieldset.S.lon[2000:][fieldset.S.lon[2000:]<0] += 360
...
def run_corefootprintparticles(dirwrite,outfile,lonss,latss,dep):
    ...
    if(config=='2pic'):
        ufiles = sorted(glob(dirread_U + 't.p21a.EO38Ma.tx0.1.2pic_control.00??????.nc'))#[:150]
        # Remove one wrongly written file:
        ufiles.remove(dirread_U + 't.p21a.EO38Ma.tx0.1.2pic_control.00400301.nc')
    elif(config=='4pic'):
        ufiles = sorted(glob(dirread_U + 't.p21a.EO38Ma.tx0.1.4pic_control.00??????.nc'))

    bfile = dirread_grid+'kmt_tx0.1_POP_EO38.nc'
    print('set the field')
    fieldset = set_the_fieldset(ufiles, bfile)
    if(area=='full_lon'):
    ...
    pset.execute(kernels, runtime=delta(days=365*7), dt=delta(minutes=-20), output_file=pfile, verbose_progress=False, recovery={ErrorCode.ErrorOutOfBounds: DeleteParticle,ErrorCode.Delete: DeleteParticle})
